(function () {
    $(function () {
        ...
        $(passwordName).on('blur', function () {
            ...
            if (result) {
                passwordResult = true;
                $(".littleone-common-section--form--error[data-name="+dataSet+"] p").text('8~20자 알파벳,숫자,특수문자');
                $(cssSelector).focus();
                $(".littleone-common-section--form--error[data-name="+dataSet+"] p").text('8~20자 알파벳,숫자,특수문자');
                $(this).val('');
                $(this).focus();
        ...
        $(rePasswordName).on('blur', function (e) {
            ...
            if (newPasswordVal !== newRePasswordVal) {
                if (newRePasswordVal.length > 0) {
                    ...
                    $(this).val('');
                    $(this).next('.validation_check').css('color', 'red').text('비밀번호가 같지 않습니다.');
                    $(this).focus();
                }
            } else {
                if (newRePasswordVal.length >= 8) {
                    rePasswordResult = true;
                    $(this).next('.validation_check').css('color', '#ea8255').text('적합');
                }
            }
